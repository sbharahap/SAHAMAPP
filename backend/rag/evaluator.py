...
2. Groundedness       : Apakah jawaban chatbot didukung penuh oleh dokumen referensi (tidak halusinasi)?
3. Answer Relevance   : Apakah jawaban chatbot menjawab inti pertanyaan user dengan tepat?
"""
...
async def evaluasi_rag_triad(query: str, contexts: list[str], response_text: str) -> dict:
    """
    Menjalankan evaluasi RAG Triad untuk menilai kualitas jawaban chatbot.
    ...
    Returns:
        Dict hasil evaluasi dengan skor 1-5 dan alasan singkat untuk masing-masing metrik.
    """
    if not contexts:
...
"""
        messages = [
            SystemMessage(content="Kamu adalah juri evaluator RAG profesional. Jawab hanya dengan format JSON valid."),
