...
struct AlertRowView: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            Image(systemName: alert.alertType == .strongBuy ? "arrow.up.circle.fill" : "exclamationmark.circle.fill")
                .font(.system(size: 30))
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(alert.isRead ? Color.primary.opacity(0.06) : signalColor.opacity(0.3), lineWidth: 1)
        )
...
struct NotificationDetailView: View {
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 4)
                        ...
                        .frame(width: geo.size.width * CGFloat(min(alert.score / 100.0, 1.0)), height: 10)
                }
            }
            .frame(height: 10)
    ...
    private func askAIAgent() {
        ...
        let name = alert.stockName.map { " (\($0))" } ?? ""
        chatbotVM.inputText = """
        Berikan analisis mendalam tentang saham \(alert.symbol)\(name)\(sectorPart) yang memiliki sinyal \(signalType) dengan skor sentimen \(Int(alert.score))/100. \
